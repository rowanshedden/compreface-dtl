...
public class FaceRecognizeProcessServiceImpl implements FaceProcessService {
    ...
    private ArrayList<FaceSimilarityDto> processFaceResult(Integer predictionCount, String apiKey, FacePredictionResultDto findResult) {
        ...
        for (val prediction : predictions) {
            ...
            /* SITA MCoE - retrieve the associated subject details */
            SubjectDetails subjectDetails = classifierPredictor.getSubjectDetails(prediction.getRight());
            SubjectDetailsDto subjectDetailsDto = new SubjectDetailsDto(subjectDetails);
            faces.add(new FaceSimilarityDto(subjectDetailsDto.getName(), pred.floatValue(), subjectDetailsDto));
            /* SITA MCoE - comment out old FaceSimilarityDto */
            //faces.add(new FaceSimilarityDto(subject, pred.floatValue()));
        }
