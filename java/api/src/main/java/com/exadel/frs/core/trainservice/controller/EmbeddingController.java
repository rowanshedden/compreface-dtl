...
public class EmbeddingController {
    ...
    public EmbeddingDto addEmbedding(
            ...
            @RequestParam(SUBJECT) final String subjectName,
    ...
    public EmbeddingDto addEmbeddingBase64(
            ...
            @Valid
            @NotBlank(message = SUBJECT_NAME_IS_EMPTY)
            @RequestParam(value = SUBJECT) final String subjectName,
            @ApiParam(value = DET_PROB_THRESHOLD_DESC, example = NUMBER_VALUE_EXAMPLE)
            @RequestParam(value = DET_PROB_THRESHOLD, required = false) final Double detProbThreshold,
            @Valid
            @RequestBody final Base64File request
    ) {
    ...
    /* SITA MCoE - create SubjectDetails API */
    @PostMapping("/faces/subject")
    public SubjectDetailsDto createSubjectDetails(
            @ApiParam(value = API_KEY_DESC, required = true)
            @RequestHeader(X_FRS_API_KEY_HEADER) final String apiKey,
            @ApiParam(value = DET_PROB_THRESHOLD_DESC, example = NUMBER_VALUE_EXAMPLE)
            @RequestParam(value = DET_PROB_THRESHOLD, required = false) final Double detProbThreshold,
            @Valid
            @RequestBody final SubjectDetailsDto request
    ) {
        imageValidator.validateBase64(request.getImage());
        ...
        EmbeddingDto embeddingDto = new EmbeddingDto(pair.getRight().getId().toString(), request.getName());
        if (embeddingDto.getEmbeddingId().isBlank() || embeddingDto.getSubjectName().isBlank() || embeddingDto.getEmbeddingId().isEmpty() || embeddingDto.getSubjectName().isEmpty())
            throw new SubjectNotFoundException(request.getName());

        return subjectService.createOrUpdateSubjectDetails(request);
    }

    /* SITA MCoE - update SubjectDetails API */
    @PostMapping("/faces/subject/update")
    public SubjectDetailsDto updateSubjectDetails(
            @ApiParam(value = API_KEY_DESC, required = true)
            @RequestHeader(X_FRS_API_KEY_HEADER) final String apiKey,
            @Valid
            @RequestBody final SubjectDetailsDto request
    ...
    public Faces listEmbeddings(
            ...
            @ApiParam(value = SUBJECT_DESC)
            @Valid
            @RequestParam(name = SUBJECT, required = false) final String subjectName,
    ...
    public List<EmbeddingDto> deleteEmbeddingsById(
            ...
            @RequestBody final List<UUID> embeddingIds
    ) {
    ...
    public VerificationResult recognizeBase64(
            ...
            @RequestParam(value = STATUS, required = false, defaultValue = STATUS_DEFAULT_VALUE) final Boolean status,
            @RequestBody
            @Valid final Base64File request
    ) {
        imageValidator.validateBase64(request.getContent());
    ...
    public EmbeddingsVerificationProcessResponse recognizeEmbeddings(
            ...
            @RequestBody
            @Valid final EmbeddingsRecognitionRequest recognitionRequest
    ) {
